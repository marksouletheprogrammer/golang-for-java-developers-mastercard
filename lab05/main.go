...
func main() {
	...
	// Create merchant store with sample data (from Lab 4)
	store := NewMerchantStore()

	// Start HTTP server on port 8080 (from Lab 4 + Lab 5 product endpoint)
...
func runPerformanceDemo() {
	...
	start := time.Now()
	enrichedSeq := EnrichProductDataSequential(products)
	...
	start = time.Now()
	enrichedConcurrent := EnrichProductDataConcurrent(products)
	...
	start = time.Now()
	enrichedFanOut := EnrichProductDataFanOut(products)
