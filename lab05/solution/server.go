...
func NewServer(store *MerchantStore) *Server {
	...
	// Register routes
	// Pattern matching: exact match for /merchants, prefix match for /merchants/
...
func (s *Server) handleMerchantByID(w http.ResponseWriter, r *http.Request) {
	...
	// r.URL.Path will be "/merchants/MERCH-001"
...
func (s *Server) handleProductEnriched(w http.ResponseWriter, r *http.Request) {
	...
	// Extract path: /products/{sku}/enriched
	...
	enriched := EnrichSingleProductFanOut(product)
